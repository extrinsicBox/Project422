...
public class PhotogrammetryHUD : Script
{
    ...
    {
        if (e.KeyCode == Keys.F12) // Changed from F11 to avoid conflict
        {
            showHUD = !showHUD;
        }
    }
    ...
    private void DrawHUD()
    {
        ...
        SharedUtilities.DrawText(string.Format("ALT: {0:F2}m", currentAGL), new PointF(boxPosition.X + globalTextOffsetX, boxPosition.Y + 50f + globalTextOffsetY), 0.2F, Color.Red);
    ...
    private void DisplayPerformanceMetrics()
    {
        ...
        {
            SharedUtilities.DrawText(string.Format("FLIGHT ACTIVE - WAYPOINT {0}/{1}", 
                SharedUtilities.CurrentWaypointIndex + 1,
                SharedUtilities.Waypoints.Count),
                new PointF(boxPosition.X + globalTextOffsetX, boxPosition.Y + 160f + globalTextOffsetY),
                0.2F, Color.Yellow);
        }
    }
    ...
    private void DrawAltitudeGraph()
    {
        ...
        for (int i = startIdx + 1; i < altitudeHistory.Count; i++)
        {
            ...
            if (i % 2 == 1) // Optimize draw calls by only drawing every other point
            {
                ...
                SharedUtilities.DrawRect(new PointF(x1, y1), new SizeF(2, 1), Color.Red);
                drawCallCount += 2;
            }
        }
    }
